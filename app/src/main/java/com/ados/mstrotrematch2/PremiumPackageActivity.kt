...
import com.ados.mstrotrematch2.databinding.ActivityPremiumPackageBinding
...
import com.ados.mstrotrematch2.util.BillingModule
import com.ados.mstrotrematch2.util.Sku
import com.ados.mstrotrematch2.util.SkuName
...
class PremiumPackageActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.buttonBuyPremiumPackage.setOnClickListener {
            ...
            mProductDetails.find { it.productId == sku }?.let { productDetails ->
                println("결제 $sku, $productDetails")
                bm.purchase(productDetails)
            } ?: also {
    ...
    private fun setInfo() {
        if (currentUser?.isPremium()!!) {
            ...
            if (currentUser?.isPremiumGemGet()!!) {
                ...
                binding.imgPremiumGemNew.visibility = View.GONE
            } else {
                ...
                binding.imgPremiumGemNew.visibility = View.VISIBLE
            }
    ...
    private fun initBilling() {
        bm = BillingModule(this, lifecycleScope, object: BillingModule.Callback {
            override fun onBillingModulesIsReady() {
                bm.querySkuDetail(BillingClient.ProductType.INAPP, Sku.PREMIUM_PACK, Sku.PREMIUM_PACK_RENEW) { skuDetails ->
                    mProductDetails = skuDetails
                }
            }

            override fun onSuccess(purchase: Purchase) {
                when {
                    Sku.PREMIUM_PACK in purchase.products -> { // 패키지 구매
                        applyPremiumPack(SkuName.PREMIUM_PACK)
                    }
                    Sku.PREMIUM_PACK_RENEW in purchase.products -> { // 패키지 갱신
    ...
    private fun setSkuDetailsView() {
        ...
        for (productDetail in mProductDetails) {
            builder.append("<${productDetail.title}>\n")
            builder.append(productDetail.oneTimePurchaseOfferDetails?.formattedPrice)
            builder.append("\n======================\n\n")
        }
