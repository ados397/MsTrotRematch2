...
data class UserDTO(
    ...
    var favorites: ArrayList<String> = arrayListOf(), // 가수 즐겨찾기
    var eventTicketIds: ArrayList<String> = arrayListOf(), // 수령한 이벤트 티켓
    var fanClubCheckoutTime: Date? = null, // 팬클럽 출석체크 시간
    var displayBoardWriteTime: Date? = null, // 전광판 마지막 등록 시간
    var token: String? = null
) : Parcelable {
    enum class LoginType {
        EMAIL, GOOGLE, FACEBOOK, NON_MEMBER
    }
    ...
    */
    ...
    fun getFanClubCheckoutImage(): Int {
        return if (isFanClubCheckout())
            R.drawable.checked
        else
            R.drawable.cancel
    }
    ...
    fun isFanClubCheckout() : Boolean {
        var isCheckout = false
        if (fanClubCheckoutTime != null) {
            var lastCheckoutTime = SimpleDateFormat("yyyy.MM.dd").format(fanClubCheckoutTime!!)
            var currentTime = SimpleDateFormat("yyyy.MM.dd").format(Date())
    ...
    // 비회원 여부
    fun isNonMember() : Boolean {
        return loginType == LoginType.NON_MEMBER
    }
}
