...
import com.ados.mstrotrematch2.databinding.MaintenanceDialogBinding
            ...
            when (jobType) {
                JobType.MAINTENANCE -> {
                    ...
                    if (!updateDTO?.maintenanceImgUrl.isNullOrEmpty()) {
                        Glide.with(context).load(updateDTO?.maintenanceImgUrl).optionalFitCenter().into(binding.imgNotice)
