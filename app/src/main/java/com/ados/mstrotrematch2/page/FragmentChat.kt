...
import com.ados.mstrotrematch2.adapter.OnChatItemClickListener
import com.ados.mstrotrematch2.adapter.RecyclerViewAdapterChat
import com.ados.mstrotrematch2.database.DBHelperReport
import com.ados.mstrotrematch2.databinding.FragmentChatBinding
...
class FragmentChat : Fragment(), OnChatItemClickListener {
    ...
    override fun onCreateView(
        ...
        firebaseViewModel.getFanClubChatsListen(fanClubDTO?.docName.toString(), userDTO?.fanClubJoinDate!!)
        firebaseViewModel.fanClubChatDTOs.observe(viewLifecycleOwner) {
            var uidSet = hashSetOf<String>()
            var isUpdateLastChatReadTime = false
            val itemsEx: ArrayList<DisplayBoardExDTO> = arrayListOf()
            var fanClubChatReadTime = sharedPreferences.getLong(MySharedPreferences.PREF_KEY_FAN_CLUB_CHAT_READ_TIME, 0L)
            var dateReadTime = Date(fanClubChatReadTime)
            for (chat in firebaseViewModel.fanClubChatDTOs.value!!) {
                if (chat.createTime != null) {
                    if (chat.createTime!! > dateReadTime) {
                        dateReadTime = chat.createTime!!
                        isUpdateLastChatReadTime = true
                    }

                    /*if (fanClubChatReadTime == 0L) {
                        fanClubChatReadTime = chat.createTime?.time!!
                        isUpdateLastChatReadTime = true
                    } else if (chat.createTime!! > dateReadTime) {
                        fanClubChatReadTime = chat.createTime?.time!!
                        isUpdateLastChatReadTime = true
                    }*/
                }
            ...
            if (isUpdateLastChatReadTime) {
                fanClubChatReadTime = dateReadTime.time!!
                sharedPreferences.putLong(MySharedPreferences.PREF_KEY_FAN_CLUB_CHAT_READ_TIME, fanClubChatReadTime)
            }
            ...
            CoroutineScope(Dispatchers.Main).launch {
                while(true) {
                    if (uriCheckIndex == uidSet.size) {
                        recyclerViewAdapter = RecyclerViewAdapterChat(itemsEx, userDTO!!, this@FragmentChat)
    ...
    private fun finishFragment() {
        val fragment = FragmentPageFanClub.newInstance(param1!!, param2!!)
