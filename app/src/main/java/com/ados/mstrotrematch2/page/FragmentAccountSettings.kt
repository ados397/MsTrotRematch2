...
import com.ados.mstrotrematch2.databinding.FragmentAccountSettingsBinding
...
class FragmentAccountSettings : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding.switchDisplayBoard.isChecked = sharedPreferences.getBoolean(MySharedPreferences.PREF_KEY_DISPLAY_BOARD_VISIBILITY, true)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.textLoginTypeInput.text = when (currentUserEx?.userDTO?.loginType) {
            UserDTO.LoginType.GOOGLE -> "구글 로그인"
            UserDTO.LoginType.EMAIL -> "이메일 로그인"
            else -> "비회원 로그인"
        }

        if (currentUserEx?.userDTO?.isNonMember()!!) { // 비회원
            binding.textLoginId.text = "복구 ID"
            binding.textLoginIdInput.text = currentUserEx?.userDTO?.uid
        ...
        binding.buttonModifyPassword.setOnClickListener {
            when (currentUserEx?.userDTO?.loginType) {
                UserDTO.LoginType.GOOGLE, UserDTO.LoginType.FACEBOOK -> {
                    callToast("소셜 로그인된 상태에서는 비밀번호를 변경할 수 없습니다.")
                }
                UserDTO.LoginType.NON_MEMBER -> {
                    callToast("비회원 로그인 사용자는 비밀번호를 변경할 수 없습니다.")
                }
                else -> {
        ...
        binding.layoutTermsOfUse.setOnClickListener {
            var dialog = WebViewDialog(requireContext(), "http://adosent.com/mstrotrematch2/terms")
        ...
        binding.layoutPrivacyPolicy.setOnClickListener {
            var dialog = WebViewDialog(requireContext(), "http://adosent.com/mstrotrematch2/privacy")
            dialog.requestWindowFeature(Window.FEATURE_NO_TITLE)
            dialog.setCanceledOnTouchOutside(false)
        ...
        binding.buttonCopyUid.setOnClickListener {
            ...
            val clip = ClipData.newPlainText("label", binding.textLoginIdInput.text)
            clipboard.setPrimaryClip(clip)
            callToast("클립보드에 복사되었습니다.")
        }

        binding.buttonJoin.setOnClickListener {
            val fragment = FragmentAccountJoin()
            parentFragmentManager.beginTransaction().apply{
                replace(R.id.layout_fragment, fragment)
                setTransition(FragmentTransaction.TRANSIT_FRAGMENT_OPEN)
                addToBackStack(null)
        ...
        binding.switchDisplayBoard.setOnCheckedChangeListener { buttonView, isChecked ->
            (activity as MainActivity?)?.setDisplayBoardVisible(isChecked)
        }
    }
