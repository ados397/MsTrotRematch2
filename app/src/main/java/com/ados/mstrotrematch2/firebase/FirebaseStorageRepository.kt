...
class FirebaseStorageRepository() {
    ...
    private val cheeringBoardImagePath = "images/cheeringBoard/"
    ...
    fun getCheeringBoardImage(seasonNum: Int, docName: String, myCallback: (Uri?) -> Unit) {
        val fileName = "${cheeringBoardImagePath}season${String.format("%04d",seasonNum)}/${docName}.jpg"
    ...
    fun setCheeringBoardImage(seasonNum: Int, docName: String, bitmap: Bitmap, myCallback: (Boolean) -> Unit) {
        val filePath = "${cheeringBoardImagePath}season${String.format("%04d",seasonNum)}/"

        var imagesRef = storageRef.child(filePath).child("${docName}.jpg")
        //이미지 파일 업로드
        imagesRef.putBytes(compressBitmap(bitmap)).addOnSuccessListener {
            myCallback(true)
        }.addOnFailureListener {
    ...
    fun deleteCheeringBoardImage(seasonNum: Int, docName: String, myCallback: (Boolean) -> Unit) {
        ...
        var imagesRef = storageRef.child(filePath).child("${docName}.jpg")    //기본 참조 위치/images/${fileName}
        //이미지 파일 삭제
        imagesRef.delete().addOnSuccessListener {
            myCallback(true)
        }.addOnFailureListener {
