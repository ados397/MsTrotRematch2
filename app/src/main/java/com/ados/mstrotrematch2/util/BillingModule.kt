...
class BillingModule(
     ...
     * @param type BillingClient.ProductType.INAPP 또는 BillingClient.ProductType.SUBS
     */
    ...
    fun querySkuDetail(
        ...
        QueryProductDetailsParams.newBuilder().apply {
            // 인앱, 정기결제 유형중에서 고름. (ProductType.INAPP, ProductType.SUBS)
            setProductList(productList)
            // 비동기적으로 상품정보를 가져옵니다.
            lifeCycleScope.launch(Dispatchers.IO) {
                val productDetailResult = billingClient.queryProductDetails(build())
                withContext(Dispatchers.Main) {
                    resultBlock(productDetailResult.productDetailsList ?: emptyList())
                }
            }
        }
    }
    ...
    /**
     * 구매 시작하기
     * @param productDetails 구매하고자하는 항목. querySkuDetail()을 통해 획득한 SkuDetail
    ...
    fun checkPurchased(
        ...
        billingClient.queryPurchasesAsync(
            ...
            if (billingResult.responseCode == BillingClient.BillingResponseCode.OK) {
                for (purchase in purchaseList) {
                    if (purchase.products.contains(sku) && purchase.isPurchaseConfirmed()) {
                        resultBlock(true)
                    }
                }
                resultBlock(false)
    ...
    private fun confirmPurchase(purchase: Purchase) {
        when {
            consumableSkus.any { it in purchase.products } -> {
