...
import com.ados.mstrotrematch2.databinding.ListItemDisplayBoardBinding
import com.ados.mstrotrematch2.model.DisplayBoardExDTO
...
class RecyclerViewAdapterDisplayBoard(private val itemsEx: ArrayList<DisplayBoardExDTO>, var clickListener: OnDisplayBoardItemClickListener) : RecyclerView.Adapter<RecyclerViewAdapterDisplayBoard.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        itemsEx[position].let { item ->
            with(holder) {
                if (!item.displayBoardDTO?.docName.isNullOrEmpty()) {
                    ...
                    ssb.apply {
                        ...
                        setSpan(ForegroundColorSpan(textColor), 0, title.text.length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE )
                    }
                    title.text = ssb
