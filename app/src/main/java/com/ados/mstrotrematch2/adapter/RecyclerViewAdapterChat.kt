...
import com.ados.mstrotrematch2.databinding.ListItemChatBinding
...
class RecyclerViewAdapterChat(private val itemsEx: ArrayList<DisplayBoardExDTO>, private val userDTO: UserDTO, var clickListener: OnChatItemClickListener) : RecyclerView.Adapter<RecyclerViewAdapterChat.ViewHolder>() {
                ...
                if (item.displayBoardDTO != null) {
                    if (item.displayBoardDTO.userUid.isNullOrEmpty()) {
                        ...
                        if (!item.displayBoardDTO.displayText.isNullOrEmpty()) {
                            ...
                            timeSystem.text = SimpleDateFormat("M월 d일 a h:mm").format(item.displayBoardDTO.createTime!!)
                        }
                    } else {
                        if (userDTO.uid == item.displayBoardDTO.userUid) { // 내가 입력한 채팅
                            layoutChatYou.visibility = View.GONE
    ...
    inner class ViewHolder(private val viewBinding: ListItemChatBinding) : RecyclerView.ViewHolder(viewBinding.root) {
        ...
        var timeSystem = viewBinding.textTimeSystem
